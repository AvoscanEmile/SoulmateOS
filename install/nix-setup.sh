...
# 3. Clear previous AVC logs to isolate Nix-related denials
ausearch --clear

# 4. Install Nix in multi-user (daemon) mode
sh <(curl --proto '=https' --tlsv1.2 -L https://nixos.org/nix/install) --daemon --yes

# 5. Capture SELinux denials and generate a custom policy module
ausearch -m avc -ts today | audit2allow -M nix-install
semodule -i nix-install.pp

# 6. Persist the Nix environment for all users via /etc/profile.d
...
# 7. Source immediately for the current session (optional)
source /etc/profile.d/nix.sh

# 8. Restore SELinux to enforcing mode (also via trap)
setenforce 1
